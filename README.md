...
- список до 100 последних исследований с локальным поиском и фильтрами;
- адаптивная карточка клинического протокола;
- переход к DICOM/OHIF;
...
- нет endpoint списка ранее созданных `user_requests`, поэтому frontend хранит
  созданные им ID в `localStorage`;
- DICOM открывается через готовую ссылку исследования либо отдельный OHIF;
