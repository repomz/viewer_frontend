...
CMD ["/bin/sh", "-c", "TLS_LISTEN=''; if [ -r /etc/viewer-tls/fullchain.pem ] && [ -r /etc/viewer-tls/privkey.pem ]; then TLS_LISTEN='listen 8443 ssl; ssl_certificate /etc/viewer-tls/fullchain.pem; ssl_certificate_key /etc/viewer-tls/privkey.pem; ssl_protocols TLSv1.2 TLSv1.3; ssl_session_cache shared:viewer_tls:10m; ssl_session_timeout 1d;'; fi; export TLS_LISTEN; envsubst '$BACKEND_URL $PACS_URL $PACS_AUTHORIZATION $TLS_LISTEN' < /etc/nginx/frontend.conf.template > /tmp/default.conf && exec nginx -c /tmp/default.conf -g 'daemon off;'"]
